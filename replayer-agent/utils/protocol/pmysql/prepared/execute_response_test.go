...
func TestDecodeStmtExecuteResonsePacket(t *testing.T) {
	var testCase = []struct {
		raw    []byte
		expect *StmtExecuteResponse
	}{
		{
			raw: []byte{
				0x01, 0x00, 0x00, 0x01, 0x04, 0x32,
	...
	for idx, tc := range testCase {
		...
		should.NoError(err)
		actual, err := DecodeStmtExecuteResonsePacket(src)
