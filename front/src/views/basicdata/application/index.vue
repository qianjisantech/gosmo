          ...
          <a-row>
            ...
            <a-col flex="0 1 400px">
              <a-form-item name="available" label="状态" style="max-width: 300px;">
                <a-select v-model:value="formState.status" placeholder="全部" allowClear>
        ...
        <a-table :rowKey="record => record.id" :columns="columns" :data-source="dataSource" :row-selection="rowSelection" :loading="tableLoading"
          ...
          <template v-slot:bodyCell="{ column, record, index }">
            ...
            <template v-if="column.dataIndex === 'status'">
              <span><a-badge :color="record.status ? 'green' : 'red'" /> {{ record.status ? '启用' :
                  '禁用' }}
              </span>
            </template>
...
import { getApplicationQueryPage } from '@/api/basicdata/application'
...
const columns: TableColumnsType = [
  ...
  {
    title: '应用名称中文',
    dataIndex: 'applicationNameCn',
    align: 'center'
  },
  {
    title: '所属产品',
    dataIndex: 'belongToProduct',
    align: 'center'
  },
  {
    title: '状态',
...
const loadingData = () => {
  ...
  getApplicationQueryPage(queryBody).then(response => {
