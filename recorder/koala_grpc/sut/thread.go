...
// OnSend OnSend
func (thread *Thread) OnSend(socketFD SocketFD, span []byte, extraHeaderSentSize int, raddr net.Addr, gid ThreadID) {
	if len(span) == 0 || thread.ignoreSocks[socketFD] {
		return
	}
	sock, err := thread.lookupSocket(socketFD)
	if sock == nil || err != nil {
		if socketFD == SocketFD(gid) {
			return
		}
		// fd 连接异常
...
func (thread *Thread) lookupSocket(socketFD SocketFD) (*socket, error) {
	...
	sock = GetGlobalGidSock(socketFD)
	if sock != nil {
		return sock, nil
	}
